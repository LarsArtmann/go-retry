...
| - | ------- | ------- | -------- |
| F1 | **Jitter breaks the MaxDelay cap.** `computeDelay` caps at `maxDelay` *before* adding up to 50% jitter, so real sleeps reach 1.5× MaxDelay while `Backoff`/`ComputeDelay` doc "capped at MaxDelay". | **CONFIRMED (bug)** | 20 000-iteration probe: max observed **299.996 ms** against declared cap 200 ms (`retry.go:150-177`) |
