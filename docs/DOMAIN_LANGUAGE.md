...
- **Attempt** — one execution of the caller's `AttemptFunc(ctx, attempt)`. The
  ...
  `Do`). The value-returning `DoWithValue` uses
  `ResultFunc[T]` with the same counter (`retry.go` `ResultFunc`).
- **`MaxAttempts`** — the **total** number of attempts, _including the first
  ...
  retries on top of the first call (`config.go` `Config`).
...
- **`DelayFunc`** — an optional callback that overrides the backoff delay for a
  ...
  default exponential backoff." (`config.go` `DelayFunc`).
- **Jitter** — random noise added _on top of_ the capped backoff delay, up to
  ...
  above the cap (`retry.go` `computeDelay`).
...
- **`WrapInfrastructure(sentinel, code, msg).WithCause(err)`** — chains the
  ...
  preserve the original cause (`retry.go` `Do`; `contextEnded`).
...
like the codes, they are part of the observable error output.
