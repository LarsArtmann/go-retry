...
- **Retryable** — an error qualifies for another attempt. Decided by
  ...
  (`retry.go:48-51`). Non-retryable errors short-circuit out of `Do`
  immediately.
...
- **`WrapInfrastructure(sentinel, code, msg).WithCause(err)`** — chains the
  ...
  `errors.Is(err, lastFnErr)` both hold (`error.go:225`, `retry.go:82-94`).
  This is how exhaustion/cancellation preserve the original cause.
