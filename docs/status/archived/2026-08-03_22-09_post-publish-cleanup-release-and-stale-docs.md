...
### ~~C1. CI workflow (`.github/workflows/ci.yml`)~~ done at `e840c7d`
...
### ~~C5. Fuzz target for `ComputeDelay`~~ done at `e840c7d`
...
### ~~C6. `hierarchical-errors` migration check~~ done (no-op confirmed 2026-08-07 — package uses `errors.Is` only)
...
### ~~C7. Public API surface audit for v1.0~~ done (tracked in ROADMAP.md v1.0 bar)
...
### ~~C8. pkg.go.dev verification~~ done (verified 2026-09-13 — v0.5.0 renders with examples)
...
### ~~C9. Go Report Card~~ Won't implement — service sunset (goreportcard.com shows a shutdown page; checked 2026-09-13)

<https://goreportcard.com/report/github.com/larsartmann/go-retry> — not
checked. Should verify the automated quality grade.

### ~~C10. README badge integration~~ Won't implement — kept badge-free (links only)
...
4. ~~**`hierarchical-errors` migration check** — confirm `go-error-family` uses~~ done (no-op confirmed 2026-08-07 (package uses errors.Is only))
   ~~no `errors.As` that needs migrating to `errors.AsType[T]` (T2). 5 minutes.~~
5. ~~**Verify pkg.go.dev rendering** — check~~ done (verified 2026-09-13 - pkg.go.dev renders v0.5.0)
   ~~`https://pkg.go.dev/github.com/larsartmann/go-retry`, request indexing if~~
   ~~stale, confirm `ExampleDo` renders.~~
6. ~~**Go Report Card** — check~~ **Won't implement — service sunset - goreportcard.com shows a shutdown page (checked 2026-09-13).**
   ~~`https://goreportcard.com/report/github.com/larsartmann/go-retry`, fix any~~
   ~~findings.~~

### P2 — Valuable

7. ~~**Add `SECURITY.md`** — vulnerability reporting policy (T5). Low effort.~~ done at `10a5566`
8. ~~**Cross-link `Backoff` ↔ `ComputeDelay`** doc comments (T4).~~ done at `10a5566`
9. ~~**README badges** — CI status, Go Report Card, pkg.go.dev, license, Go~~ **Won't implement — kept badge-free (links only).**
   ~~version reference.~~
10. ~~**Public API surface audit** — confirm every exported symbol is one~~ done (tracked in ROADMAP.md (v1.0 bar))
    ~~callers should depend on before v1.0 (ROADMAP).~~
11. ~~**Options-based configuration investigation** — functional options~~ done (tracked in ROADMAP.md (options-based configuration))
    ~~(`WithOnRetry`, `WithMaxAttempts`) so new fields don't break struct-literal~~
    ~~callers. Design doc first (ROADMAP).~~
12. ~~**`AttemptFunc` signature review** — some retry libraries pass the~~ done (tracked in ROADMAP.md (v1.0 bar))
    ~~previous error back into `fn`; this one does not. Deliberate decision or~~
    ~~accident? (ROADMAP v1.0 question).~~
13. ~~**Configurable jitter factor** — currently hardcoded at 50% of delay;~~ **Won't implement — deferred (decision 2026-08-08; see ROADMAP.md).**
    ~~consider making it a `Config` field (FEATURES WORTH_CONSIDERING).~~
14. ~~**Deterministic RNG option** — allow callers to inject an `*rand.Rand`~~ done (tracked in FEATURES.md WORTH_CONSIDERING)
    ~~for testable backoff (FEATURES WORTH_CONSIDERING).~~
15. ~~**Version-compatibility matrix with `go-error-family`** — document which~~ done (tracked in ROADMAP.md (version-compatibility matrix))
...
19. ~~**Issue templates** — `.github/ISSUE_TEMPLATE/` for bug reports and~~ **Won't implement — no external contributors yet.**
    ~~feature requests.~~
20. ~~**PR template** — `.github/PULL_REQUEST_TEMPLATE.md`.~~ **Won't implement — no external contributors yet.**
21. ~~**`CODEOWNERS`** — `.github/CODEOWNERS` for review routing.~~ **Won't implement — solo maintainer.**
22. ~~**Discussions tab** — enable GitHub Discussions for Q&A.~~ **Won't implement — solo maintainer.**
23. ~~**Go version badge in README** — `go.mod` says 1.26.5; badge should match.~~ **Won't implement — README states the Go requirement in text; badge-free by choice.**
24. ~~**GitHub Actions cache** — if CI uses `actions/setup-go`, enable module~~ done (setup-go cache enabled (ci.yml))
    ~~caching for faster runs.~~
25. ~~**Dependabot config** — `.github/dependabot.yml` for automated dependency~~ done (present (.github/dependabot.yml))
    ~~bumps (`go-error-family`, `golangci-lint` action, `actions/setup-go`).~~
26. ~~**Release automation** — `goreleaser` config or a GitHub Action that~~ **Won't implement — releases cut per the go-release skill.**
    ~~cuts tags + releases from commit messages.~~
27. ~~**Changelog automation** — consider `conventional-commits` + auto-generated~~ **Won't implement — CHANGELOG curated manually per Keep-a-Changelog.**
    ~~CHANGELOG for future releases.~~
28. ~~**Sitemap / SEO** — if a docs site is built (ROADMAP), set up~~ done (not applicable without a docs site)
    ~~`sitemap.xml` and meta tags.~~
29. ~~**Favicon / social card** — if a docs site is built, create OG image and~~ done (not applicable without a docs site)
    ~~favicon.~~
30. ~~**`doc.go` link to domain glossary** — cross-reference~~ **Won't implement — doc.go stays minimal; glossary linked from README/CONTRIBUTING.**
    ~~`docs/DOMAIN_LANGUAGE.md` from the package doc comment.~~

### P4 — Exploration / future

31. ~~**Public documentation site** — Astro + Starlight + Firebase Hosting~~ done (tracked in ROADMAP.md (public documentation site))
    ~~(ROADMAP; `website-launch` skill). Precondition: API stability.~~
32. ~~**Circuit-breaker adapter** — thin adapter that composes with a~~ done (tracked in ROADMAP.md (composition primitives))
    ~~caller-chosen breaker. Likely belongs in `go-cqrs-lite`, not here (ROADMAP).~~
33. ~~**Bulkhead / deadline-budgeting primitives** — same: likely document the~~ done (tracked in ROADMAP.md (composition primitives))
    ~~pattern, don't code it here (ROADMAP non-goal).~~
34. ~~**Metrics hooks** — `OnRetry`/`OnExhausted` are the seam; consider a~~ **Won't implement — OnRetry/OnExhausted are the seam; metrics wiring is consumer-side.**
    ~~documented example of wiring Prometheus metrics through them.~~
35. ~~**Structured logging example** — show how to wire `slog` through~~ **Won't implement — consumer-side; no demand.**
    ~~`OnRetry`/`OnExhausted`.~~
36. ~~**`context.AfterFunc` migration** — Go 1.21+ offers `context.AfterFunc`;~~ **Won't implement — select on timer.C vs ctx.Done() is the simpler, proven form.**
...
37. ~~**`MaxDelay` as a `time.Duration` vs `int64`** — currently `time.Duration`~~ done (overflow-safe (B3 float-space compare + saturation; matrix test))
    ~~(correct), but confirm all delay arithmetic is overflow-safe for extreme~~
    ~~`Multiplier` values.~~
38. ~~**Heap allocations in the hot path** — benchmark `Do` end-to-end (not just~~ done (BenchmarkComputeDelay reports 0 allocs/op)
    ~~`ComputeDelay`) to confirm zero allocations per attempt.~~
39. ~~**Race detector coverage** — run `go test -race -count=100` to catch~~ done (go test -race -count=10 green (suite convention))
    ~~rare data races in the backoff/jitter path.~~
40. ~~**Module proxy verification** — `GOPROXY=off go mod download` to confirm~~ done (proxy + go get verified (22:09 A6; re-verified for v0.4.0/v0.5.0))
    ~~the module is fully cached and resolvable without network.~~
41. ~~**`go mod tidy` in CI** — ensure `go.sum` stays in sync.~~ **Won't implement — not added; single-dependency module, tidy run at release.**
42. ~~**License scan** — some organizations require automated license scanning~~ **Won't implement — no org requirement.**
    ~~(FOSSA, Snyk). Consider adding a badge/config.~~
43. ~~**Tidelift** — if monetizing open source, consider Tidelift listing.~~ **Won't implement — not pursued.**
44. ~~**GitHub Sponsors** — `FUNDING.yml` if the author wants sponsorships.~~ **Won't implement — not pursued.**
45. ~~**Semantic release tags** — document the tagging convention~~ **Won't implement — tagging convention lives in the go-release skill.**
    ~~(`vMAJOR.MINOR.PATCH`, signed, annotated) in CONTRIBUTING.~~
46. ~~**Backward compatibility tests** — `apicompat` or similar to catch~~ done (tracked in ROADMAP.md (v1.0 bar))
    ~~breaking API changes before they ship.~~
47. ~~**Golden file tests for error messages** — ensure error codes/messages~~ done (error codes asserted in tests; DOMAIN_LANGUAGE table is the contract)
    ~~don't drift across versions (they're part of the public contract).~~
48. ~~**Integration test with `go-cqrs-lite`** — confirm the `MessageAdapter`~~ done (consumer documented (doc.go; go-cqrs-lite/middleware/v4))
    ~~in `middleware/v4` still compiles against this package's API.~~
49. ~~**`doc.go` test coverage** — `go test` doesn't cover `doc.go`; consider~~ **Won't implement — godoclint linter guards doc references; doc.go is 9 lines.**
    ~~a `TestPackageDoc` that asserts the package comment doesn't reference~~
    ~~removed types.~~
50. ~~**Archive prior status reports** — `docs/status/` now has 3 reports;~~ done (docs-health pass 2026-09-13 (reports archived to docs/status/archived/))
