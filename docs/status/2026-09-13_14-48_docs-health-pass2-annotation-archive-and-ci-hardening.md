...
2. **I cited a commit hash from file-stat inference; it was wrong.** T9's
   ...
   failure class as the 12:20 report's d.6, which I had *read hours earlier*
   and still repeated.
...
4. **One wasted lint round trip**: wrote a >120-char `t.Fatalf` line; lint
   ...
   avoided it.
5. **Luck-adjacent ordering**: TODO_LIST cites the *archived* paths of the two
...
19. `[DOC]` DOMAIN_LANGUAGE: decide the policy for *external* citations
